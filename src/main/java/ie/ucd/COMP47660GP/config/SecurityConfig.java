    ...
    public void configure(HttpSecurity http) throws Exception {
        ...
        http.csrf().csrfTokenRepository(CookieCsrfTokenRepository.withHttpOnlyFalse()).and().authorizeRequests()
                .antMatchers("/register", "/", "/login", "/secureRegister", "/secureLogin", "/resources/**",
                        ...
                        "/editCreditCardDetails", "/registerCard", "/editPersonalDetails", "/flight",
                        "/create-reservation", "/createMember", "/create-reservation/*", "/deleteAccount", "/success",
                        "/fail", "/error")
                .permitAll().anyRequest().authenticated().and().formLogin().loginPage("/login")
                .loginProcessingUrl("/secureLogin").usernameParameter("username").passwordParameter("password").and()
                .logout().logoutSuccessUrl("/logoutSuccess").invalidateHttpSession(true).deleteCookies("SESSID")
                .permitAll().and().sessionManagement().sessionCreationPolicy(SessionCreationPolicy.IF_REQUIRED)
                .sessionFixation().newSession().invalidSessionUrl("/login");
